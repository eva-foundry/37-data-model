...
#
# Adapted from 51-ACA sprint_agent.py -- full sprint cloud execution runner
# Executes FK Enhancement sprints from GitHub Issues labeled "sprint-task"
#
# Usage:
#   python3 sprint_agent.py --issue N --repo owner/repo
#
# Sprint issue format: issue body must contain JSON manifest block:
#   <!-- SPRINT_MANIFEST
#   { ... }
#   -->
#   See .github/sprints/sprint-NN-name.md for schema.
...
# GitHub Models endpoint -- GITHUB_TOKEN in CI grants access to gpt-4o, gpt-4o-mini
MODEL = "gpt-4o"
GITHUB_MODELS_URL = "https://models.inference.ai.azure.com"
STATE_FILE = REPO_ROOT / "sprint-state.json"
SUMMARY_FILE = REPO_ROOT / "sprint-summary.md"

# Data model integration (cloud: ACA, local dev: port 8010)
...
def update_story_status(story_id: str, status: str, **kwargs) -> bool:
    ...
    for key in ["sprint_id", "epic_id", "title", "commit_sha", 
...
_SIBLING_MAP: dict[str, list[str]] = {
    # Validation layer changes may need cosmos context
    "api/validation.py": [
        "api/cosmos.py",
        "api/layers.py",
    ],
    # Router changes need cosmos + validation
    "api/routers/": [
        "api/cosmos.py",
        "api/validation.py",
        "api/layers.py",
    ...
    # Script changes may need cosmos + validation
    "scripts/": [
        "api/cosmos.py",
        "api/validation.py",
    ],
...
def _load_context() -> str:
    ...
    This function provides:
      ...
      3. USER-GUIDE.md excerpt (data model usage patterns)
      4. FK Enhancement roadmap (Phase 0-6 overview)

    USER-GUIDE.md is the cross-sprint feedback loop: it documents patterns
    established in prior phases so each new sprint does not regenerate the same bugs.
    """
    lines = []
    lines.append("PROJECT: 37-data-model -- EVA Data Model (Single Source of Truth API)")
    lines.append("STACK: Python 3.12 / FastAPI / Azure Cosmos DB NoSQL / Azure Container Apps")
    lines.append("PHASE: FK Enhancement (12 sprints, 7 phases, 403h, 83 FP)")
    lines.append("")
    ...
    userguide_path = REPO_ROOT / "USER-GUIDE.md"
    if userguide_path.exists():
        userguide_lines = userguide_path.read_text(encoding="utf-8", errors="replace").splitlines()
        lines.append("")
        lines.append("=== USER-GUIDE.md (data model usage patterns -- FOLLOW THESE) ===")
        lines.extend(userguide_lines[:120])  # cap at 120 lines (~300 tokens)
    fk_plan_path = REPO_ROOT / "docs" / "FK-ENHANCEMENT-EXECUTION-PLAN-2026-03-01.md"
    if fk_plan_path.exists():
        fk_lines = fk_plan_path.read_text(encoding="utf-8", errors="replace").splitlines()
        lines.append("")
        lines.append("=== FK Enhancement Roadmap ===")
        lines.extend(fk_lines[:40])  # Phase summary only
    return "\n".join(lines)
...
def _generate_code(story: dict, context: str) -> dict[str, str]:
    """
    Call GitHub Models gpt-4o to generate file contents for a story.
    ...
    system_prompt = textwrap.dedent("""
        You are an expert senior developer implementing FK Enhancement stories for the
        37-data-model (EVA Data Model -- single source of truth API).
        You have full codebase context. Follow ALL rules exactly from copilot-instructions.md.
        ...
        - Python / YAML / Dockerfile: # EVA-STORY: F37-NN-NNN
        - JS / TS / Bicep:            // EVA-STORY: F37-NN-NNN
        - HTML / JSX / TSX:           <!-- EVA-STORY: F37-NN-NNN -->
        - Tag MUST appear on the first functional line of every file.

        CODE PATTERNS (from copilot-instructions P2.5 -- apply exactly):

        Pattern 1 -- Data model layer validation (MANDATORY):
          All PUT/POST to /model/* must validate foreign keys:
          - endpoint.service must exist in services layer
          - screen.api_calls must reference valid endpoint IDs (exact "METHOD /path")
          - container.partition_key must match field name in .fields array
          - hook.calls_endpoints must reference valid endpoint IDs
          Reject invalid references with 400 + specific error message.

        Pattern 2 -- Cosmos row versioning (MANDATORY for conflict resolution):
          Every PUT must:
          1. GET existing record first
          2. Preserve row_version from GET
          3. Strip audit columns (modified_by, modified_at, etc.) before PUT
          4. Server increments row_version on successful write
          5. Concurrent PUT with stale row_version returns 409 Conflict

        Pattern 3 -- HTTP API patterns (FastAPI):
          - PUT /model/{layer}/{id} -- upsert one object
          - GET /model/{layer}/ -- list all objects in layer
          - GET /model/{layer}/{id} -- get one object
          - POST /model/admin/validate -- run FK validation across all 31 layers
          - POST /model/admin/commit -- export model/*.json + validate + Cosmos sync
          Never use SQLite queries directly -- only Cosmos API via api/cosmos.py.

        Pattern 4 -- Test patterns (pytest):
          - All tests in tests/ directory
          - Use @pytest.fixture for setup/teardown
          - Test file naming: test_<module>.py
          - Assert on specific error messages, not just status codes
        ...
        - FK Enhancement: Focus on validation, not schema changes.
          Do not modify layer schemas without explicit instruction.
...
# ---------------------------------------------------------------------------
# Check step (ruff + pytest collect)
# ---------------------------------------------------------------------------
...
def commit_story(story: dict, written_files: list, evidence_path: str) -> str:
    ...
    _git(["config", "user.name", "37-data-model Sprint Agent"])
    _git(["config", "user.email", "agent+data-model@eva-foundry.dev"])
...
4. Run model validation: POST /model/admin/validate (violation_count must be 0)
5. Export to Cosmos: POST /model/admin/commit (exported_total > 0)
...
def run_sprint(issue: int, repo: str) -> None:
    ...
    sprint_full_id = f"37-data-model-{sprint_id.lower()}"
...
Working through {len(stories)} stories in sequence. Progress comments will follow after each story.
    ...
    try:
        pr_result = _run([
            "gh", "pr", "create",
            "--repo", repo,
            "--title", f"feat({sprint_id}): {manifest.get('sprint_title', '')}",
            "--body", summary,
            "--base", "main",
            "--head", branch,
        ])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="37-data-model sprint agent -- full sprint executor")
