...
class CacheConfig:
    ...
    MEMORY_CACHE_TTL = int(os.getenv('CACHE_TTL_MEMORY_SECONDS', '120'))  # 2 minutes
    REDIS_CACHE_TTL = int(os.getenv('CACHE_TTL_REDIS_SECONDS', '1800'))   # 30 minutes
...
class CacheManager:
        ...
        try:
            ...
            if CacheConfig.REDIS_ENABLED:
                try:
                    ...
                    if connected:
                        logger.info(f"Redis connected to {CacheConfig.REDIS_HOST}:{CacheConfig.REDIS_PORT}")
                    else:
                        logger.warning("Redis connection failed, continuing with memory cache only")
    ...
    def stats(self) -> dict:
        ...
        stats = {
            ...
            'redis_connected': bool(self.redis_client and self.redis_client.is_connected),
