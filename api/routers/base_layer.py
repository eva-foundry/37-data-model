...
def make_layer_router(layer: str, prefix: str, tags: list[str]) -> APIRouter:
    ...
    async def list_objects(
        ...
        custom_filters = {k: v for k, v in query_params.items() if k not in reserved_params}
        ...
        if custom_filters:
            ...
            sample_obj = next((obj for obj in data if not obj.get("id", "").endswith("...")), None)
            if sample_obj:
            ...
            for obj in data:
                ...
                for param_name, param_value in custom_filters.items():
                    # Parse operator from param name (e.g., "count.gt" -> ("count", "gt"))
                    if "." in param_name:
                    ...
                    if field not in valid_fields:
                        if param_name not in [w["param"] for w in query_warnings]:
                            query_warnings.append({
                                "param": param_name,
                                "message": f"Field '{field}' not found in {layer} schema",
                                "valid_fields": sorted(list(valid_fields)[:20]),  # show first 20 fields
                    ...
                    # Get field value from object (support nested fields with dot notation)
                    obj_value = obj
                    for key_part in field.split("."):
                        if isinstance(obj_value, dict) and key_part in obj_value:
                    ...
                    if operator == "eq":
                        ...
                        if f"{param_name}.{operator}" not in [w["param"] for w in query_warnings]:
                            query_warnings.append({
                                "param": param_name,
