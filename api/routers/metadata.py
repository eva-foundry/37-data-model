...
async def get_layer_metadata(
    ...
    priority: Optional[str] = Query(
        None,
        regex="^(P0|P1|P2|P3|P4)(,(P0|P1|P2|P3|P4))*$",
    ...
    sort: str = Query(
        "layer_number",
        regex="^(layer_number|layer_name|priority|category|operational)$",
        description="Sort field"),
    with_fk: Optional[bool] = Query(
        None,
        description="Filter layers with FK relationships")):
    """
    ...
    if operational is not None:
        filtered_layers = [layer for layer in filtered_layers if layer.get(
            "operational") == operational]

    if priority:
        priorities = priority.split(",")
        filtered_layers = [
            layer for layer in filtered_layers if layer.get("priority") in priorities]

    if category:
        filtered_layers = [
            layer for layer in filtered_layers if layer.get("category") == category]

    if with_fk is not None:
        if with_fk:
            # Has FK relationships (either outbound or inbound)
            filtered_layers = [
                layer for layer in filtered_layers if (
                    layer.get("fk_references") and len(
                        layer.get(
                            "fk_references",
                            [])) > 0) or (
                    layer.get("referenced_by") and len(
                        layer.get(
                            "referenced_by",
                            [])) > 0)]
        else:
            ...
            filtered_layers = [
                layer for layer in filtered_layers if (
                    not layer.get("fk_references") or len(
                        layer.get(
                            "fk_references",
                            [])) == 0) and (
                    not layer.get("referenced_by") or len(
                        layer.get(
                            "referenced_by",
                            [])) == 0)]
    ...
    # Sort
    sort_key = {
        "layer_number": lambda x: x.get(
            "layer_number", 999), "layer_name": lambda x: x.get(
            "layer_name", ""), "priority": lambda x: x.get(
                "priority", "P9"), "category": lambda x: x.get(
                    "category", ""), "operational": lambda x: (
                        0 if x.get("operational") else 1, x.get(
                            "layer_number", 999))}.get(
                                sort, lambda x: x.get(
                                    "layer_number", 999))
    ...
    # Calculate metadata
    operational_count = sum(1 for layer in layers if layer.get("operational"))
    ...
    return {
        ...
        "metadata": {
            ...
            "stub_count": len(layers) -
            operational_count,
            "last_updated": metadata_index.get(
                "last_updated",
                "2026-03-08T18:58:00Z"),
...
async def get_layer_metadata_by_name(layer: str):
    ...
    if layer.isdigit():
        layer_num = int(layer)
        found = next((lyr for lyr in layers if lyr.get(
            "layer_number") == layer_num), None)
        if found:
            return found

    # Try to find by name
    found = next((lyr for lyr in layers if lyr.get("layer_name") == layer), None)
