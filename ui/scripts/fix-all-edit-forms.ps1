#!/usr/bin/env pwsh
# fix-all-edit-forms.ps1
# Batch fix for all EditForm components - replace Mustache placeholders with minimal working code
    ...
    try {
        ...
        if ($hasPlaceholders) {
            ...
            if (-not $DryRun) {
                ...
                $content = $content -replace '(?s)// Initialize form data with existing record values.*?\}\, \[record\]\);', @'
...
'@
                ...
                # Fix hasChanges section
                $content = $content -replace '(?s)const hasChanges = \(\): boolean => \{.*?\{\{/EDITABLE_FIELDS\}\}.*?return false;.*?\};', @'
...
if ($DryRun) {
    ...
    Write-Host "`n[COMPLETE] All EditForm components fixed" -ForegroundColor Green
}
